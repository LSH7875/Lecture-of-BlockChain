...
//최초의 접속
function wsInit(){
    const server = new WebSocket.Server({ port:wsPORT })//자기 자신의 도메인에서 포트만 넣어서 생성해준거임
    //내 자신의 호스트로 해당 포트를 열겠다.
    server.on("connection",(ws)=>{//커넥션이 완료가 되면 콜백함수를 실행하겠다.
...
function connectionToPeers(newPeers){
    ...
    newPeers.forEach(peer=>{
        ...
        const ws = new WebSocket(peer)//도메인가지 받은 것임 요청을 보내겠다..
        ws.on("open",()=>{
            init(ws)
        })
        ws.on("error",()=>{
